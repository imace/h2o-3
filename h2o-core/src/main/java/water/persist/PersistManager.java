...
//      }
  ...
  public void importFiles(String path, String pattern, ArrayList<String> files, ArrayList<String> keys, ArrayList<String> fails, ArrayList<String> dels) {
    ...
    if(!pattern.isEmpty()) {
      ArrayList<String> subsetFiles = matchPattern(files,pattern); //New files ArrayList after matching pattern of choice
      files.retainAll(subsetFiles); //Subset files based on subsetFiles
      ArrayList<String> subsetKey = matchPattern(keys,pattern); //New keys ArrayList after matching pattern of choice
   ...
   */
  public ArrayList<String> matchPattern(ArrayList<String> fileList, String matchStr){
    ArrayList<String> result = new ArrayList<String>();
    Pattern pattern = Pattern.compile(matchStr);
    if (matchStr != null) {
      for(String d : fileList){
        Matcher matcher = pattern.matcher(d.substring(d.lastIndexOf("[/\\\\\\\\]") + 1).trim());
        if (matcher.find()) {
          result.add(d);
        }
      }
